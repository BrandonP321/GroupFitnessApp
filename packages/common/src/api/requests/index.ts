...
export type ProtectedAPIRequest<T extends APIRequestResponse> = (urlParams: T["UrlParams"], bodyParams: T["ReqBody"], headers: T["headers"], haveUserReAuth: () => void, getRefreshToken: () => Promise<string | null>, storeTokens: (at: string, rt: string) => Promise<boolean>) => (
    Promise<AxiosResponse<T["ResBody"]>>
)
...
/**
 * This API route wrapper function attempts to execute a protected API route, which would require the client to provide an access token
 * to access that endpoint.  If the token is expired, this will attempt to refresh both the access & refresh tokens, and then also attempt to
 ...
 * a custom client side function is called to redirect the user to a login screen for re-authentication.
 * 
