...
export class StorageUtils {
    ...
    private static storeItem(key: ValidSecureStorageKey | ValidAsyncStorageKey, value: any, secure?: boolean) {
        return new Promise<boolean>(async (resolve, reject: (resason: boolean) => void) => {
            try {
 ...
 * EXAMPLES
 * 
 * setSecureItemAsync(AsyncStorageKeys.testKey.key, { fizz: "buzz" }).then(res => res).catch((e: IStorageErr) => e.msg)
 * 
 * getSecureItemAsync(SecureAsyncStorageKeys.testKey.key).then(res => res).catch((e: IStorageErr) => e.msg)
 * 
 * getItemAsync(AsyncStorageKeys.testKey.key).then(res => res).catch((e: IStorageErr) => e.msg)
 * 
 * setItemAsync(AsyncStorageKeys.testKey.key, { fizz: "buzz" }).then(res => res).catch((e: IStorageErr) => e.msg)
 */
